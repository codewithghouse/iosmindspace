...
export default function EmailScreen({ isVisible, onContinue, onNavigateToSignIn }: { isVisible: boolean; onContinue: (email: string, password: string) => void; onNavigateToSignIn?: () => void }) {
        ...
        <div className="text-center max-w-sm w-full">
          ...
          <form onSubmit={handleSubmit} className="w-full" aria-label="Create account form">
            <div className="relative mb-4 transition-all duration-300">
              ...
              />
              {emailError && (
                <p id="email-error" className="mt-2 text-xs text-red-500 font-light" role="alert">
                  {emailError}
                </p>
              )}
            </div>
            ...
            <div className="relative mb-2 transition-all duration-300">
              ...
              <input
                ...
                aria-describedby={passwordError || showPasswordStrength ? 'password-feedback' : undefined}
                className={`w-full py-4 px-5 rounded-full border transition-all duration-300 shadow-sm touch-target theme-transition ${
                  passwordError 
                    ? 'border-red-400 focus:border-red-500 focus:ring-1 focus:ring-red-500' 
                    : isPasswordFocused
              ...
              />
              {passwordError && (
                <p className="mt-2 text-xs text-red-500 font-light" role="alert">
                  {passwordError}
                </p>
              )}
            </div>

            {showPasswordStrength && password && !passwordError && (
              <div id="password-feedback" className="mb-4" role="status" aria-live="polite">
                <div className="flex items-center gap-2 mb-1">
                  <div className="flex-1 h-1 rounded-full overflow-hidden theme-transition" style={{ backgroundColor: borderColor }}>
                    <div
                      className={`h-full transition-all duration-300 ${
                        passwordStrength.strength === 'weak' ? 'bg-red-400 w-1/4' :
                        passwordStrength.strength === 'fair' ? 'bg-orange-400 w-2/4' :
                        passwordStrength.strength === 'good' ? 'bg-yellow-400 w-3/4' :
                        'bg-green-500 w-full'
                      }`}
                    />
                  </div>
                ...
                {passwordStrength.feedback && (
                  <p className="text-xs font-light mt-1 theme-transition" style={{ color: textSecondary }}>
                    {passwordStrength.feedback}
                  </p>
                )}
              </div>
